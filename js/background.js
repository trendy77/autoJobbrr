...
var popupJsPort = "port-popup";
var contentJsPort = "port-content";
...
chrome.runtime.onConnect.addListener(popListener, true);
chrome.runtime.onConnect.addListener(conListener, true);

function popListener(port) {
	console.assert(port.name == popupJsPort);
...
function conListener(port) {
	console.assert(port.name == contentJsPort);
	port.onMessage.addListener(function(msg) {
		if (typeof(msg.response) !== undefined) {
			return;
		} else if (typeof(msg.state) !== 'undefined') {
...
chrome.contextMenus.onClicked.addListener(function(item, tab) {
	...
	if (tit == 'Send2Sheet') {
		...
		chrome.tabs.create({
			url: chrome.extension.getURL("tabs_api.html")
		});
...
function changeState(newState) {
	...
	switch (fstate) {
		...
		case STATE_AUTHTOKEN_ACQUIRED:
			chrome.contextMenus.update('RevokeToken', {
				visible: true
			});
			chrome.contextMenus.update('SignIn', {
				visible: false
			});
			chrome.contextMenus.update('ResetFields', {
				visible: true
			});
			chrome.contextMenus.update('Send2Sheet', {
				visible: true
			});
			chrome.contextMenus.update('OpenOptions', {
				visible: true
			});
			break;
	}
	...
	function executionAPIResponse(response) {
		...
		function revokeAuthTokenCallback(current_token) {
			if (!chrome.runtime.lastError) {
				...
				xhr.send();
				changeState(STATE_START);
				sendLog('Token revoked and removed from cache. ' +
		...
		*/
		 ...
		 */
		function createElement(tagName, owner, opt_position, opt_size,
			opt_noAppend) {

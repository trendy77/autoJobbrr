...
let parent = chrome.contextMenus.create({"title": "jobSEEKr"});
for (let i = 0; i < theTitles.length; i++) {
  let context = theTitles[i];
  let title =  context.title + ' :' + context.value;
	let id = chrome.contextMenus.create({"title": title,  "parentId": parent, "id": title, context:[selection],
                                       "onclick": upContext});
}
...
let radio1 = chrome.contextMenus.create({"title": "Radio 1", "type": "radio",
                                         "onclick":radioOnClick});
let radio2 = chrome.contextMenus.create({"title": "Radio 2", "type": "radio",
                                         "onclick":radioOnClick});
